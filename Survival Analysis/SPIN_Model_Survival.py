...
class SPIN(nn.Module):
    def __init__(self, net_hparams, pathway_idx)):
        ...
        ###########################################################################
        self.layer3 = nn.Linear(net_hparams[1][1], net_hparams[2], bias=False)
    ...
    def forward(self, x, e, t):
        ...
        if (0 not in x[:, -1]) and (1 not in x[:, -1]):
            ...
            if 0 in x[:, -1]:
                ...
                events.append(e[idx_female])
                times.append(t[idx_female])
            if 1 in x[:, -1]:
                ### force the connections between gene layer and pathway layer w.r.t. 'pathway_mask'
                ...
                events.append(e[idx_male])
                times.append(t[idx_male])
